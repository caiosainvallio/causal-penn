...
# (Z) IV is nearc4 (near 4 year college)
...
#is the IV associated with the treatment? strength of IV
...
table(predtx) # = mean(educ12[card.data$nearc4==1]); mean(educ12[card.data$nearc4==0])

# stage 2: regress Y on predicted value of A
summary(lm(card.data$lwage ~ predtx))
